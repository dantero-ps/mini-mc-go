...
IMPORTANT: Import "crypto/sha256" and "math/rand" (for test RNG with fixed seed, NOT for game logic). Keep all existing tests unchanged.
  ...
  <verify>
Run `go test ./internal/world/... -v -count=1` and verify all tests pass. Run `go test ./internal/world/... -bench=BenchmarkDensity -benchmem` to get performance baseline. All determinism tests must pass with 0 failures.
  </verify>
